...
if ( _truck_to_unload getVariable ["KLT_ammo_truck_load", 0] > 0 ) then {
    _truck_to_unload setVariable ["KLT_ammo_truck_load", 0, true];
    [_truck_to_unload, false] remoteExec ["KLT_fnc_protectObject"];

    {
        _next_box = _x;
        [_next_box, false] remoteExec ["KLT_fnc_protectObject"];
        sleep 0.5;
        detach _next_box;
        ...
        _offset = _offset - 2.2;
        sleep 0.5;
        [_next_box] remoteExec ["KLT_fnc_protectObject"];
        [_next_box, true] remoteExec ["enableRopeAttach"];
    } foreach ( attachedObjects _truck_to_unload);

    sleep 0.5;

    [_truck_to_unload] remoteExec ["KLT_fnc_protectObject"];

    hint localize "Box Unloaded";
    uiSleep 2;
    hint "";
};
