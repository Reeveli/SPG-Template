 ...
 * Client side function to handle map dialog onload. Called from the dialog onUnload event
 *
 * Arguments:
 * 0: Dialog <DISPLAY>
 ...
 *
 * Example:
 * [_display,_exitCode] call Rev_arty_fnc_gunship_map_dialog_onUnload
 *
 */
...
if !(_exitCode == 1) then {player setVariable ['Rev_arty_gun_call',nil]};

//Getting Dialog current inputs here because markers need to be deleted early in the script
private _loiter = sliderPosition 6043;
private _altitude = sliderPosition 6045;
private _view_distance = sliderPosition 6049;
private _start_pos = getMarkerPos 'Rev_arty_gun_dir';

//removing map EH
removeMissionEventHandler ['MapSingleClick', missionNameSpace getVariable ['Rev_arty_gun_map_event',-1]];
//deleting markers
deleteMarkerLocal 'Rev_arty_gun_tgt';
deleteMarkerLocal 'Rev_arty_gun_area';
deleteMarkerLocal 'Rev_arty_gun_dir';
deleteMarkerLocal 'Rev_arty_gun_arrow_1';
deleteMarkerLocal 'Rev_arty_gun_arrow_2';
deleteMarkerLocal 'Rev_arty_gun_arrow_3';
...
_args pushBack "B_T_VTOL_01_armed_F";
_args pushBack _loiter;
_args pushBack _altitude;
_args pushBack _view_distance;
