 ...
 * Client side function to handel initial resupply request from artillery dialog_exit.
 *
 * Arguments:
 ...
 *
 * Example:
 * [_location,_target,_range,_angle,_number,_caller_pos,"Supply",_delay] call Rev_arty_fnc_type_SUP
 *
1.3
...
if (Rev_arty_SUP_amount == 0) exitWith {[{playSound "FD_Start_F"; hint "No supply drops available!";}, [], 0.001] call CBA_fnc_waitAndExecute;};
if !(_number == 1) exitWith {[{playSound "FD_Start_F"; hint "Select 1 as the round amount!";}, [], 0.001] call CBA_fnc_waitAndExecute;};
if ((_number <= Rev_arty_SUP_amount) && (_number != 0) && (_number == _unmodified_number)) then {hintSilent "";};
...
if !(isNil {player getvariable ['Rev_arty_sup_call',nil]}) exitWith {[{playSound "FD_Start_F"; hint "Previous supply request is still processing!";}, [], 0.001] call CBA_fnc_waitAndExecute;};

[{[] call Rev_arty_fnc_supply_dialog},[], 0.001] call CBA_fnc_waitAndExecute;
