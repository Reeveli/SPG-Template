 ...
 * Client side function to handel initial illumination support request from artillery dialog_exit.
 *
 * Arguments:
...
1.5
	Max dinstance hit updated to use addonoption
1.4
...
if (_number > Rev_arty_ILM_amount) then {_number = Rev_arty_ILM_amount; [{playSound "FD_Start_F"; params ["_ammo_amount"];hint format ["Maximum available ordance of %1 used!",_ammo_amount];}, [Rev_arty_ILM_amount], 0.001] call CBA_fnc_waitAndExecute;};
if (_number == 0) exitWith {[{playSound "FD_Start_F"; hint "No rounds selected!";}, [], 0.001] call CBA_fnc_waitAndExecute;};
if ((_number <= Rev_arty_ILM_amount) && (_number != 0) && (_number == _unmodified_number)) then {hintSilent "";};
