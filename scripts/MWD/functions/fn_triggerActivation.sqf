...
if (!isNull _dog && _unit == _dog) then {
	...
	// If we reached this point, the dog is currently NOT tracking
	...
	evId = addMissionEventHandler ["Draw3D", {				
		...
		_trackIcon = getMissionPath "scripts\mwd\sfx\track.paa";		
		//_trackLostIcon = getMissionPath "scripts\mwd\sfx\track_lost.paa";
	...
	// Check to see if the dog is in heel mode (NOT allowed to track)
	_isHeel = _dog getVariable ["MWD_Status", ""] == "HEEL";
	if (_isHeel) exitWith {
		// Add 3D marker for in-game positions (let the handler know there's a track here)
		_dog setVariable ["MWD_Markers3D", []];
		(_dog getVariable ["MWD_Markers3D", []]) pushBack [getPos _dog, "Trace?"];		
	};
	
	// Let's get it into tracking mode!
	...
	// Add 3D marker for in-game positions
	(_dog getVariable ["MWD_Markers3D", []]) pushBack [getPos _dog, "Trace found"];
